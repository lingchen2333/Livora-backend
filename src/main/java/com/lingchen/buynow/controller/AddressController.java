...
import com.lingchen.buynow.request.CreateAddressListRequest;
...
public class AddressController {
    ...
    public ResponseEntity<ApiResponse> createAddressList(@RequestBody CreateAddressListRequest request) {
        List<AddressDto> addressDtoList = addressService.createAddressList(request.getAddressList())
                .stream().map(addressService::convertToDto)
                .toList();

        return ResponseEntity.ok(new ApiResponse("Address list created successfully", addressDtoList));
    }
    ...
    public ResponseEntity<ApiResponse> updateAddressById(@PathVariable Long addressId, @RequestBody UpdateAddressRequest request) {
        ...
        return ResponseEntity.ok(new ApiResponse("Address with id " + addressId + " updated successfully", null));
    }
}
